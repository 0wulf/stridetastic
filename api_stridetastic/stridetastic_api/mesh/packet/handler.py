...
from ...models import Channel, Edge, Interface, Node, NodeLatencyHistory, NodeLink
...
try:
    # Exposed for tests that patch `stridetastic_api.mesh.packet.handler.ServiceManager`.
...
def _decimal_from(
    value: Optional[float | int], *, places: Optional[int] = None
) -> Optional[Decimal]:
    if value is None:
        return None
    try:
...
def _update_latency_history(
    *,
    ...
    if pending_entry is None:
        pending_entry = (
            history_qs.filter(
                reachable=False, latency_ms__isnull=True, responded_at__isnull=True
            )
            .order_by("time")
            .first()
        )
...
def handle_nodeinfo(payload: bytes, packet_data: PacketData) -> None:
    ...
    pubkey = (
        base64.b64encode(user.public_key).decode("utf-8") if user.public_key else None
    )
    logging.info(f"[NodeInfo]\n{user}")
    logging.info(
        f"[NodeInfo] node_num={node_num}, mac_address={macaddr}, hw_model={hw_model}, role={role}, public_key={pubkey}"
    )
...
def handle_neighborinfo(payload: bytes, packet_data: PacketData) -> None:
    ...
    if neighbor_info.last_sent_by_id:
        ...
        try:
            ...
            logging.debug(
                f"[NeighborInfo] Invalid last_sent_by_id {neighbor_info.last_sent_by_id}"
            )

    neighbor_payload, _ = NeighborInfoPayload.objects.get_or_create(
        packet_data=packet_data
    )
    ...
    neighbor_payload.node_broadcast_interval_secs = (
        ...
        if neighbor_info.node_broadcast_interval_secs
        else None
    )
    ...
    for advertised in neighbor_info.neighbors:
        ...
        if neighbor_node_num is not None:
            try:
                ...
                logging.debug(
                    f"[NeighborInfo] Invalid neighbor node num {neighbor_node_num}"
                )
        ...
        broadcast_interval = (
            advertised.node_broadcast_interval_secs
            if advertised.node_broadcast_interval_secs
            else None
        )
...
def handle_position(payload: bytes, packet_data: PacketData) -> None:
    ...
    logging.info(
        f"[Position] lat={pos.latitude_i/1e7}, lon={pos.longitude_i/1e7}, alt={pos.altitude}, time={pos.time}"
    )
...
def handle_telemetry(payload: bytes, packet_data: PacketData) -> None:
    ...
    try:
        ...
        if telemetry.HasField("device_metrics"):
            ...
            voltage = (
                round(device_metrics.voltage, 2) if device_metrics.voltage else None
            )
            channel_utilization = (
                round(device_metrics.channel_utilization, 2)
                if device_metrics.channel_utilization
                else None
            )
            air_util_tx = (
                round(device_metrics.air_util_tx, 2)
                if device_metrics.air_util_tx
                else None
            )
            logging.info(
                f"[Telemetry] device_metrics: battery_level={device_metrics.battery_level}, voltage={voltage}, channel_utilization={channel_utilization}, air_util_tx={air_util_tx}, uptime_seconds={device_metrics.uptime_seconds}"
            )
            telemetry_payload, _ = TelemetryPayload.objects.get_or_create(
                packet_data=packet_data,
            )
            telemetry_payload.battery_level = (
                device_metrics.battery_level if device_metrics.battery_level else None
            )
        ...
        if telemetry.HasField("environment_metrics"):
            ...
            temperature = (
                round(env_metrics.temperature, 2) if env_metrics.temperature else None
            )
            relative_humidity = (
                round(env_metrics.relative_humidity, 2)
                if env_metrics.relative_humidity
                else None
            )
            barometric_pressure = (
                round(env_metrics.barometric_pressure, 2)
                if env_metrics.barometric_pressure
                else None
            )
            gas_resistance = (
                round(env_metrics.gas_resistance, 2)
                if env_metrics.gas_resistance
                else None
            )
            iaq = round(env_metrics.iaq, 2) if env_metrics.iaq else None
            logging.info(
                f"[Telemetry] environment_metrics: temperature={temperature}, relative_humidity={relative_humidity}, barometric_pressure={barometric_pressure}, gas_resistance={gas_resistance}, iaq={iaq}"
            )
            telemetry_payload, _ = TelemetryPayload.objects.get_or_create(
                packet_data=packet_data,
            )
...
def handle_route_discovery(payload: bytes, packet_data: PacketData) -> None:
    ...
    broadcast_present = broadcast_present or any(
        node_num == BROADCAST_NODE_NUM for node_num in route_back_nums
    )
    ...
    if packet_data.request_id == 0:
        ...
        if ackd_packet:
            ...
            if broadcast_present:
                forward_has_known_intermediate = any(
                    node is not None for node in route_nodes[1:-1]
                )
                back_has_known_intermediate = any(
                    node is not None for node in route_back_nodes[1:-1]
                )
                if not (forward_has_known_intermediate or back_has_known_intermediate):
                    return
            ...
            def build_edge_segments(
                nodes: list[Optional[Node]], snr_values: list[float]
            ) -> list[tuple[Node, Node, Optional[float], int]]:
                ...
                for index, node in enumerate(nodes):
                    ...
                    if source_node is not None and target_node is not None:
                        segments.append(
                            (source_node, target_node, snr_value, hop_count)
                        )
            ...
            logging.info(
                f"[Routing] Creating edges for route: {route_node_list}, {route_nodes}, SNR: {route_snr_list}"
            )
            forward_segments = build_edge_segments(route_nodes, route_snr_list)
            persist_edge_segments(forward_segments)

            logging.info(
                f"[Routing] Creating edges for route back: {route_node_back_list}, {route_back_nodes}, SNR: {snr_back_list}"
            )
...
def handle_decoded_packet(
    ...
    port = (
        portnums_pb2.PortNum.Name(decoded_data.portnum)
        if decoded_data.portnum in portnums_pb2.PortNum.values()
        else None
    )
    want_response = (
        decoded_data.want_response if hasattr(decoded_data, "want_response") else None
    )
    ...
    data_obj.reply_id = (
        decoded_data.reply_id if hasattr(decoded_data, "reply_id") else None
    )
    ...
    logging.info(
        f"[Packet] from: {from_node_number} ({from_node_id}, {from_node_shortname}, {from_node_longname}) >-- port:{port} --> to: {to_node_number} ({to_node_id}, {to_node_shortname}, {to_node_longname})"
    )
...
def handle_packet(
    ...
    if packet.HasField("decoded"):
        ...
        if not pki_encrypted:
            if key is not None:
                ...
                if payload is not None:
                    ...
                    logging.info("[Encrypted] Could not decrypt packet")
            else:
                logging.info("[Encrypted] No key provided for decryption.")
        else:
            ...
            try:
                ...
                pki_service = (
                    manager.get_pki_service() or manager.initialize_pki_service()
                )
        ...
        logging.info("[Unknown] Packet has no decoded or encrypted payload.")
        logging.info(f"[Unknown] Packet:\n{packet}")
    packet_obj.raw_data = (
        base64.b64encode(packet.encrypted).decode("utf-8")
...
def _dispatch_to_publisher_service(
...
def on_message(client, userdata, normalized, iface="MQTT"):
    interface_id = (
        normalized.get("interface_id") if isinstance(normalized, dict) else None
    )
    if interface_id:
    ...
    if target_for_edge is not None:
        link_edge, _ = Edge.objects.get_or_create(
            source_node=from_node, target_node=target_for_edge
        )
        link_edge.last_packet = packet_obj  # type: ignore[assignment]
    ...
    logging.info(
        f"[Packet] from: {from_node_num} ({from_node_id}, {from_node_mac}) >----> to: {to_node_num} ({to_node_id}, {to_node_mac})"
    )
