...
from .pki_service import PKIEncryptionResult, PKIService
...
class PublisherService:
    ...
    def publish(
        ...
        logging.debug(
            f"[PublisherService.publish] Called with publisher={publisher is not None}, base_topic={base_topic}, gateway_node_id={gateway_node_id}, channel_name={channel_name}"
        )

        active_publisher = publisher or self._publisher
        if not active_publisher:
            logging.error(
                f"[PublisherService.publish] No publisher configured (explicit={publisher is not None}, default={self._publisher is not None})"
            )
            raise RuntimeError("No publisher configured for PublisherService")

        logging.debug(
            "[PublisherService.publish] Active publisher selected, checking connection..."
        )
        if not active_publisher.is_connected():
            logging.warning(
                "[PublisherService.publish] Publisher not connected, cannot send published message"
            )
            return False
        ...
        logging.debug(
            f"[PublisherService.publish] publisher.publish() returned: {publish_success}"
        )

        if publish_success:
            logging.info(
                f"[PublisherService.publish] Successfully published published message to topic: {topic}"
            )
            return True
        else:
            logging.warning(
                f"[PublisherService.publish] Publish failed for topic: {topic}"
            )
            return False
    ...
    def _encrypt_pki_payload(
        ...
        *,
        ...
        source_node = (
            Node.objects.filter(node_id=from_node_id)
            .only("private_key", "node_num")
            .first()
        )
        if not source_node or not source_node.private_key:
            raise ValueError(
                "Source node does not have PKI private key material on record"
            )
        ...
        target_node = (
            Node.objects.filter(node_id=to_node_id)
        ...
        if target_node and target_node.public_key:
            try:
                ...
                raise ValueError(
                    f"Stored recipient public key is invalid: {exc}"
                ) from exc
        ...
        try:
            to_node_num = (
                int(target_node.node_num)
                if target_node and target_node.node_num is not None
                else id_to_num(to_node_id)
            )
        except (TypeError, ValueError):
        ...
        result: PKIEncryptionResult = self._pki_service.encrypt_packet(
            inputs, source_node.private_key
        )
        ...
        *,
        ...
        with self._reactive_lock:
            ...
            if trigger_ports is not None:
                ...
                for name in trigger_ports:
                    ...
                    try:
                        ...
                        logging.warning(
                            f"[Publisher] Ignoring unknown port name in reactive config: {name}"
                        )
                        continue
            ...
            if listen_interface_ids is not None:
                ...
                missing_ids = {
                    iface_id
                    for iface_id in listen_interface_ids
                    if iface_id not in found_ids
                }
                if missing_ids:
                    logging.warning(
    ...
    def _resolve_publish_context(
        self, interface_obj: Optional[Interface]
    ) -> Tuple[Optional[PublishableInterface], Optional[str]]:
        ...
        try:
            from .service_manager import (  # local import to avoid circular dependency at module load
                ServiceManager,
            )
            ...
            logging.debug(
                f"[Publisher] Unable to resolve publish context via ServiceManager: {exc}"
            )
            return fallback_publisher, fallback_topic
        ...
        if not wrapper:
            logging.debug(
                f"[Publisher] Interface {interface_pk} not active; falling back to default publisher"
            )
            return fallback_publisher, fallback_topic
        ...
        if publisher is None:
            logging.debug(
                f"[Publisher] Runtime wrapper for interface {interface_pk} missing implementation; using fallback publisher"
            )
            return fallback_publisher, fallback_topic
        ...
        if hasattr(publisher, "is_connected"):
            try:
                if not publisher.is_connected():
                    logging.debug(
                        f"[Publisher] Interface {interface_pk} publisher not connected; using fallback publisher"
                    )
                    return fallback_publisher, fallback_topic
            except Exception as exc:  # pragma: no cover - defensive logging
                logging.debug(
                    f"[Publisher] Failed to check publisher connection state: {exc}"
                )
                return fallback_publisher, fallback_topic
    ...
    def publish_text_message(
        ...
        logging.info(
            f"[Publisher] Publishing message from {from_node} to {to_node}: {message_text}"
        )
    ...
    def publish_nodeinfo(
        ...
        logging.info(
            f"[Publisher] Publishing nodeinfo from {from_node} to {to_node}: "
    ...
    def publish_position(
        ...
        logging.info(
            f"[Publisher] Publishing position from {from_node} to {to_node}: "
    ...
    def publish_telemetry(
        ...
        logging.info(
            f"[Publisher] Publishing telemetry from {from_node} to {to_node}: type={telemetry_type}, fields={list(telemetry_options.keys())}"
        )
        data_pb = craft_telemetry(
        ...
        *,
        ...
        if payload_type == "position" or job.payload_type == job.PayloadTypes.POSITION:
            ...
            if lat is None or lon is None:
                raise ValueError(
                    "Latitude and longitude are required for periodic position payloads"
                )
        ...
        if payload_type == "nodeinfo" or job.payload_type == job.PayloadTypes.NODEINFO:
            ...
            missing = [
                key for key, value in required_fields.items() if value in (None, "")
            ]
            if missing:
                raise ValueError(
        ...
        if (
            ...
            or job.payload_type == job.PayloadTypes.TRACEROUTE
        ):
        ...
        if (
            ...
            or job.payload_type == job.PayloadTypes.TELEMETRY
        ):
        ...
        raise ValueError(
            f"Unsupported periodic publish payload type: {job.payload_type}"
        )
    ...
    def on_packet_received(
        ...
        with self._reactive_lock:
            ...
            if not config.from_node:
                logging.debug(
                    "[Publisher] Reactive config missing source node, skipping traceroute injection"
                )
                return
            ...
            listen_filter_ids = (
                set(listen_filter_qs.values_list("id", flat=True))
                if listen_filter_qs.exists()
                else set()
            )
            ...
            if listen_filter_ids:
                selected_interface = next(
                    (iface for iface in interfaces if iface.pk in listen_filter_ids),
                    None,
                )
                if selected_interface is None:
                    logging.debug(
                        "[Publisher] Packet interface not in reactive listener set; skipping traceroute"
                    )
                    return
            else:
            ...
            try:
                ...
                logging.debug(
                    f"[Publisher] Failed to resolve channel for reactive traceroute: {exc}"
                )
                channel_obj = None

            if channel_obj is None or not getattr(channel_obj, "channel_id", None):
                logging.debug(
                    "[Publisher] Unable to determine channel from packet; skipping traceroute"
                )
                return
            ...
            if trigger_ports:
                if decoded_data is None or portnum is None:
                    logging.debug(
                        "[Publisher] Packet had no decoded portnum; skipping reactive traceroute"
                    )
                    return
                try:
                    ...
                    logging.debug(
                        f"[Publisher] Unknown port number {portnum}; skipping reactive traceroute"
                    )
                    return
            ...
            if not publisher:
                logging.debug(
                    "[Publisher] No publisher available for reactive traceroute"
                )
                return
            ...
            try:
                ...
                logging.error(
                    f"[Publisher] Failed to inject reactive traceroute: {exc}"
                )
